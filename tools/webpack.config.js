...
const reImage = /\.(bmp|gif|jpg|jpeg|png|svg|ttf|woff|woff2)$/;
...
// function isComponent(module, name) {
  ...
  module: {
    ...
    rules: [
      // Rules for JS / JSX
      {
        ...
        exclude: [
          resolvePath(SRC_DIR, 'data/migrations'),
          resolvePath(SRC_DIR, 'data/seeders'),
          resolvePath(SRC_DIR, 'data/seeders-media'),
        ],
        loader: "babel-loader",
        options: {
      ...
      {
        exclude: [reScript, reGraphql, reStyle, reImage, /\.json$/, /\.txt$/, /\.md$/],
        loader: "file-loader",
        options: {
          name: staticAssetName,
        },
  ...
  optimization: {
    ...
    minimizer: [
      new UglifyJSPlugin({
        uglifyOptions: {
          compress: {
            drop_console: !isDebug, // removes console.logs from build
          }
        }
      })
    ],
