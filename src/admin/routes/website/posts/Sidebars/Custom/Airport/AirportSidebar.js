...
class AirportSidebar extends Component {
  ...
  componentDidMount() {
    const { posts, post: { meta: { airport_list } } } = this.props;
    if (this.filterOriginalPosts(posts, airport_list).length === 0) {
  ...
  componentWillReceiveProps(nextProps) {
    const { posts, post: { meta: { airport_list } } } = nextProps;
    this.setState({
    ...
    if (!this.state.postFetched && nextProps.posts.length) {
      this.setState({
        postFetched: true,
        selectedPosts: this.filterOriginalPosts(posts, airport_list)
      });
    }
  }
  ...
  addPost = item => {
    ...
    this.props.onMetaChange({
      ...
      airport_list: selectedPosts.map(post => ({ post_uuid: post.id }))
    });
    console.log('====> ', this.props.post.meta.airport_list);
  };
  ...
  filterOriginalPosts = (posts, corporate_list) =>
    posts.filter(v =>
      (corporate_list || []).map(_v => _v.post_uuid).indexOf(v.id) !== -1)
    ...
    return (
      <div className="pt-form-group">
        <label className="pt-label" htmlFor="selectedPosts">
          <TooltipLabel label="Airport Lists" required tooltip="Please select posts" />
          <MultiSelect
            name="airport_list"
