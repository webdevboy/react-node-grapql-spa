...
class ServiceListSidebar extends Component {
  constructor(props) {
    super(props);
    ...
    return (
      <div className="pt-form-group">
        <label className="pt-label" htmlFor="selectedPosts">
          <TooltipLabel label="Services List" required tooltip="Please select posts" />
          <MultiSelect
            name="corporate_list"
...
ServiceListSidebar.contextTypes = {
    ...
    posts: (Object.values(state.posts.byId) || []).filter(post => {
      ...
      if (post.taxonomies) {
        const foundTax = post.taxonomies.find(
          tax => tax.taxonomy === "article_category" && tax.term && tax.term.name === "Services",
        );
        if (foundTax) {
          rightTerm = true;
        }
...
export default connect(mapStateToProps, { fetchPosts, fetchPostByTaxonomyTerm })(withStyles(s)(ServiceListSidebar));
